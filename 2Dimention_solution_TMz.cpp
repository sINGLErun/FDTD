...
//-------- modeling constants -----------------------------------------------------------
	...
	const double gauss_w_sec = 2e-10;					// width of gaussian signal
		...
		for (int i = 1; i < SIZE_X-2; ++i) {
			for (int j = 1; j < SIZE_Y-2; ++j) {
				...
				Hz[i][j] += + ((Ex[i][j+1] - Ex[i][j]) -
							   (Ey[i+1][j] - Ey[i][j])) * dt/(mu0*d);
			}
		}

//-------- find Ex[i+1/2,j] at t+1 ------------------------------------------------------
		for (int i = 1; i < SIZE_X-1; ++i) { // if you want to use ABC you need to have field component defined at the boundary
...
//-------- find Ey[i,j+1/2] at t+1 ------------------------------------------------------
		for (int i = 1; i < SIZE_X-1; ++i) {
			for (int j = 1; j < SIZE_Y-1; ++j) { // if you want to use ABC you need to have field component defined at the boundary
				assert((i-1)>=0 && "Ey[i,j+1/2] Out of boundary");
				Ey[i][j] += + (Hz[i][j] - Hz[i-1][j]) * dt/(eps[i][j]*eps0*d);
			}
		}

		Ey[SRC_POS_X][SRC_POS_Y] += exp(-(t-gauss_d)*(t-gauss_d) / (gauss_w*gauss_w));

		write_2Darray(Ey, SIZE_X, SIZE_Y-1, fout);
	}
