...
const int DET_NUM = 5;
const int DET_POS[DET_NUM] = {0, SRC_POS, SIZE_X/2-5, SIZE_X/2+5, SIZE_X-1};
...
//void sigma_init(float*, int SIZE);
void losses_init(float*, int SIZE);
void CE_init(float*, int SIZE);
void CH_init(float*, const float* eps, int SIZE);

int main(int argc, char const *argv[])
{
	float Hy[SIZE_X-1], Ez[SIZE_X], eps[SIZE_X], CE[SIZE_X], CH[SIZE_X];
	std::ofstream fout_det[DET_NUM];

	component_init(Hy, Ez, SIZE_X-1); Ez[SIZE_X-1] = 0;
	dielectric_init(eps, SIZE_X);
	CE_init(CE, SIZE_X); CH_init(CH, eps, SIZE_X);
	...
	for (int t = 0; t < TIME_m; ++t) {
		...
		for (int i = 1; i < SIZE_X-1; ++i)
			Ez[i] = CE[i]*Ez[i] + CH[i]*(Hy[i] - Hy[i-1]);
...
void losses_init(float* eps, int SIZE) {
	for (int i = 0; i < SIZE/2; ++i)	
		eps[i] = 0;
	for (int i = SIZE/2; i < SIZE; ++i)
		eps[i] = 0.01;	
...
void CE_init(float* CE, int SIZE) {
	float loss[SIZE]; losses_init(loss, SIZE);
	for (int i = 0; i < SIZE; ++i)
		CE[i] = (1 - loss[i]) / (1 + loss[i]);
}

void CH_init(float* CH, const float* eps, int SIZE) {
	float loss[SIZE]; losses_init(loss, SIZE);
	for (int i = 0; i < SIZE; ++i)
		CH[i] = W0 / (eps[i]*(1 + loss[i]));	
}
