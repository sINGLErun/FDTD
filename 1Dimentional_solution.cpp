...
const unsigned TIME_m = 1000;
const unsigned DET_POS = 50;
const unsigned SRC_POS = 40;
...
{
	float Hy[SIZE_X], Ez[SIZE_X];
	std::ofstream fout_det, fout_source;

	component_init(Hy, Ez, SIZE_X);
	...
	for (int t = 0; t < TIME_m; ++t) {
		...
		// find Hy[] at t+1/2
		Hy[SIZE_X-1] = Hy[SIZE_X-2]; // absorbing boundary conditions
		for (int i = 0; i < SIZE_X-1; ++i) // x = i*dx;
			Hy[i] += (Ez[i+1] - Ez[i])*Sc / W0;

		Hy[SRC_POS - 1] += -exp(-(t - 30.0)*(t - 30.0)/100.0);
		...
		Ez[SRC_POS] += exp(-(t+1 - 30.0)*(t+1 - 30.0)/100.0);
